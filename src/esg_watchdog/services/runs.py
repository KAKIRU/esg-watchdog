"""pipeline_runs 기록 헬퍼 — collect · extract · detect · match · score · publish 가 전부 여기 하나만 쓴다.

- trigger 는 전부 'manual' (cron 없음, D-28). status 는 success / partial / failed.
- DB import 는 함수 안에서만 한다 — .env 없이도 서비스 모듈을 import 할 수 있어야 한다(테스트는 가짜 client 주입).
"""
...
def decide_status(succeeded: int, failed: int) -> str:
    """단위 작업(회사·문서·배치)의 성공/실패 수로 실행 상태를 정한다. 실패 0 → success, 성공 0 → failed, 섞이면 partial."""
    if failed == 0:
        return "success"
    if succeeded == 0:
        return "failed"
...
def failure_note(failed_labels: Sequence[str]) -> str | None:
    """pipeline_runs.note — 실패한 단위 목록. 없으면 None."""
