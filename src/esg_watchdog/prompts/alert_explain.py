...
- 금지 표현(knowledge/banned_terms.yaml)은 프롬프트에 category 6개와 예시를 넣고, 적재 전에 banned_terms.check_fields 로 다시 검사한다.
  걸리면 regeneration_hint 를 붙여 1회 재생성. 길이(400~800자)가 벗어나면 길이 힌트를 붙여 1회 재생성.
"""
...
[limitation] 1~2문장. 판단의 근거와 한계. 사건이 확정되지 않았으면(confirmed=false) "조사·의혹 단계로 확정되지 않았습니다" 취지의 문장을 반드시 넣는다.
[fallback] 공약·사건을 다르게 읽을 수 있는 대안 해석 1문장. 없으면 null.

[소급 건] is_retrospective 가 true 면 ②에서 "공시 이전에 발생했으나 공시 이후 보도로 확인된 사안" 임을 밝힌다.

[금지 — 반드시 지킨다] 매매 판단 · 가격 전망 · 확정적 단정 · 사법 판단 단정을 쓰지 않는다. 다음 금지 표현 분류에 해당하는 말을 피한다:
